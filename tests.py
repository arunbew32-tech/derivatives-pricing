...
def finite_difference_vega_test(S_0, r,T,X,sigma,q, option_type):
    approx_vega = (black_scholes(S_0, r,T,X,sigma+0.01,q, option_type) - black_scholes(S_0, r,T,X,sigma-0.01,q, option_type))/2*0.01
    assert abs(vega(S_0, r,T,X,sigma,q)- approx_vega) < 1e-4 

def finite_difference_rho_test(S_0, r,T,X,sigma,q, option_type):
    approx_rho = (black_scholes(S_0, r+0.001,T,X,sigma,q, option_type) - black_scholes(S_0, r-0.001,T,X,sigma,q, option_type))/(2*0.001)
